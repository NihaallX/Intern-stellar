...
from src.scrapers.startupjobs import scrape_startupjobs_jobs
...
def export_csv(jobs: list[Job], output_dir: str = "data") -> str:
    ...
    csv_path = Path(output_dir) / "jobs_history.csv"
    ...
    # Open in append mode so multiple runs accumulate in one history file.
...
def run_pipeline(
    ...
    if settings.startupjobs.get("enabled", True):
        try:
            sj_jobs = scrape_startupjobs_jobs(
                max_results=settings.startupjobs.get("max_results", 40),
                mode=settings.startupjobs.get("mode", "rss"),
            )
    ...
    from src.llm.nim_client import infer_posted_date
    ...
    for job in all_jobs:
        ...
        # If no parseable date, ask NIM fallback to infer from text
        if job.posted_date is None:
            inferred = infer_posted_date(job.title, job.description, job.url)
            if inferred:
        ...
        if is_job_too_old(job.posted_date):
            stale_count += 1
            print(f"  [FRESHNESS] EXCLUDE (too old): {job.title} (posted {job.posted_date.strftime('%Y-%m-%d')})")
            continue

        # Check for stale heuristics (old year references, closed postings)
        if is_likely_stale(job.title, job.description):
            stale_count += 1
            print(f"  [FRESHNESS] EXCLUDE (heuristic stale): {job.title}")
            continue

        # If reached here, job is considered fresh
        print(f"  [FRESHNESS] KEEP: {job.title} (posted {job.posted_date.strftime('%Y-%m-%d')})")
