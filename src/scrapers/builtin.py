        ...
        for idx, query in enumerate(search_queries):
            try:
                ...
                for result in response.get("results", []):
                    ...
                    # Extract company from title (handle "Role at Company", "Company - Role", "Title | Company")
                    company = "Unknown Company"
                    clean_title = title
                    role_indicators = [
                        "product manager", "associate product manager", "apm", "engineer",
                        "intern", "junior", "senior", "principal", "lead", "product",
                    ]

                    # Try common separators and pick which side is company vs role
                    for sep in [" at ", " | ", " - "]:
                        if sep in title:
                            ...
                            if len(parts) >= 2:
                                ...
                                if left_is_role and not right_is_role:
                                    ...
                                    # Default: assume left is title, right is company
                                    clean_title = parts[0]
                                    company = parts[1]
                            break
