...
def _extract_company(title: str, url: str) -> str:
    ...
    # Heuristic: split on common separators, but determine which side is company vs title
    role_indicators = [
        "product manager", "associate product manager", "apm", "engineer", "forward deployed",
        "solutions engineer", "intern", "junior", "senior", "principal", "lead", "product",
    ...
    for sep in [" at ", " - ", " | "]:
        if sep in title:
            ...
            if len(parts) >= 2:
                ...
                # If left looks like a role (contains role indicators) and right doesn't, swap
                left_is_role = any(ind in left_lower for ind in role_indicators)
                right_is_role = any(ind in right_lower for ind in role_indicators)
                ...
                if left_is_role and not right_is_role:
                    ...
                    # Fallback: prefer the shorter component as company (often company names are shorter)
                    company_candidate = left if len(left) < len(right) else right

                if 2 < len(company_candidate) < 100:
