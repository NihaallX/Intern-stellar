...
```yaml
tavily:
  api_key: ""  # Prefer TAVILY_API_KEY in the environment
  enabled: true
  enrich_companies: true
  max_enrichment_jobs: 4  # Only enrich top N to save API calls
```
...
tavily:
  ...
  max_enrichment_jobs: 4     # Limit to top N jobs (saves API calls)
```
