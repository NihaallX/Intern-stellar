...
RATE_LIMIT_DELAY = 4.0  # seconds between API calls (increased to be safer)
MAX_RETRIES = 5
RETRY_BASE_DELAY = 10  # seconds, doubles each retry
RETRY_JITTER = 2.0  # seconds max jitter to add to backoff
        ...
        try:
            with httpx.Client(timeout=30.0) as client:
                ...
                if response.status_code == 429:
                    ...
                    jitter = RETRY_JITTER * (0.5 + 0.5 * (attempt / max(1, MAX_RETRIES - 1)))
                    retry_wait = retry_after + (jitter * (0.5 - (time.time() % 1)))
                    print(f"[LLM] Rate limited (429), switched key + waiting {retry_wait:.0f}s (attempt {attempt + 1}/{MAX_RETRIES})")
...
def extract_flags_with_llm(job: Job) -> Optional[ExtractedFlags]:
    ...
    """
    try:
        # Allow forced fallback to avoid long Groq rate-limit stalls during runs
        if os.getenv('FORCE_LLM_FALLBACK'):
            print('[LLM] FORCE_LLM_FALLBACK set — skipping Groq call and using fallback')
            return None
